...
public class ConfigCache {
	...
	public static String getUrlCache(String url) {
		...
		if (file.exists() && file.isFile()) {
			...
			if (Application.mNetWorkState != NetUtil.NETWORK_NONE
					&& expiredTime < 0) {
				return null;
			}
			if (Application.mNetWorkState == NetUtil.NETWORK_WIFI
					&& expiredTime > CONFIG_CACHE_WIFI_TIMEOUT) {
				return null;
			} else if (Application.mNetWorkState == NetUtil.NETWORK_MOBILE
